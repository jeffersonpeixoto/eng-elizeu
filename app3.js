...
// ... todas suas funções acima ...
async function enviarPushFCM({ topic, title, body, data }) {
...
async function processarEventoChamado(chamado, tipo) {

  const regrasSnap = await admin.firestore()
    .collection("regras_notificacao")
    .where("evento", "==", tipo)
    .get();
  ...
  for (const doc of regrasSnap.docs) {
    ...
    // 🔥 filtro inteligente
    const bate = Object.entries(regra.condicoes || {})
      .every(([k, v]) => chamado[k] === v);
